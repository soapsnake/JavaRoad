...
public class Question1192 {
    ...
    //except its parent and return a smallest timestamp in all its neighbors. If this timestamp is
    //strictly less than the node's timestamp, we know that this node is somehow in a cycle.
    //Otherwise, this edge from the parent to this node is a critical connection
    public List<List<Integer>> criticalConnections(int n, List<List<Integer>> connections) {
        List<Integer>[] graph = new ArrayList[n];  //头一回见这么奇葩的数据结构
    ...
    public void criticalConnectionsUtil(List<Integer>[] graph, int parent, int node, int[] timer,
                                        boolean[] visited, List<List<Integer>> results, int[] timeStampAtThatNode) {
        visited[node] = true;
