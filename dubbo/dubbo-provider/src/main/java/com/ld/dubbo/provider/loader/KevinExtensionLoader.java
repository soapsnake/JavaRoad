package com.ld.dubbo.provider.loader;

import com.ld.dubbo.provider.annotation.LogSpi;
import com.ld.dubbo.provider.util.Holder;
 ...
 * @date 2018/10/29
 */
public class KevinExtensionLoader<T> {


    /**
     * 定义SPI文件的扫描路径,dubbo源码中设置了多个,我们这里只设置一个路径就够了
     */
    private static final String EXTENSION_DIRECTORY = "META-INF/logger/";
     ...
     * 拓展点加载器的缓存
     */
    private static final ConcurrentMap<Class<?>, KevinExtensionLoader<?>> EXTENSION_LOADERS =
    ...
    public static <T> KevinExtensionLoader<T> getExtensionLoader(Class<T> type) {
        ...
        if (!withExtensionAnnotation(type)) {//3.需要添加spi注解,否则抛异常
            throw new IllegalArgumentException("Extension type(" + type +
                    ") is not extension, because WITHOUT @" + LogSpi.class.getSimpleName() + " Annotation!");
        }
        //从缓存EXTENSION_LOADERS中获取,如果不存在则新建后加入缓存
        //对于每一个拓展,都会有且只有一个ExtensionLoader与其对应
        KevinExtensionLoader<T> loader = (KevinExtensionLoader<T>) EXTENSION_LOADERS.get(type);
        if (loader == null) {
            EXTENSION_LOADERS.putIfAbsent(type, new KevinExtensionLoader<>(type));
            loader = (KevinExtensionLoader<T>) EXTENSION_LOADERS.get(type);
        }
        return loader;
    }
    ...
    private Map<String, Class<?>> loadExtensionClasses() {
        final LogSpi defaultAnnotation = type.getAnnotation(LogSpi.class);
